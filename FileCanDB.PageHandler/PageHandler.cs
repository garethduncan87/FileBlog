...
namespace FileCanDB.PageHandler
{
    public class PageHandler
    {
        ...
        public PageHandler(string DatabaseLocation)
        {
            this._databaseLocation = DatabaseLocation;
            FileCanDbPosts = new FileCanDB<T>(this._databaseLocation, "Blogs", "Articles", true, StorageType.json);
            Regex rgx = new Regex("[^a-zA-Z0-9 -]");
            string CacheName = rgx.Replace(DatabaseLocation, "");
            this._cache = new MemoryCache(CacheName);
        }
        ...
        /// <returns></returns>
        public bool Save(T page, string Name)
        {
            if (FileCanDbPosts.Update(Name, page))
            {
                Cache();
                return true;
            }
        ...
        public bool Create(T page, string Name)
        {
            if (FileCanDbPosts.Insert(Name, page))
            {
                Cache();
                return true;
            }
        ...
        {
            T page = FileCanDbPosts.Read(Title).Data;
            page.Archive = true;
        ...
        public PostModel Load(string Title)
        {
            ...
            if (MemoryCache.[Title] == null)
                Cache();

            if (_cache.Get(Title) != null)
